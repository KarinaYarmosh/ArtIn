...
class Grid:
    ...
    def create_grid(self, size):
        ...
        for i in range(len(matrix)):
            ...
            for j in range(len(matrix[i])):
                ...
                if matrix[i][j] == 2 and (
                        matrix[i][j - 1] == 2 or matrix[i - 1][j - 1] == 2 or matrix[i - 1][j] == 2 or matrix[i][
                    j - 1] == 2):
    ...
    def draw_grid(self, grid_matrix, window):
        ...
        for i in range(len(grid_matrix)):
            ...
            for j in range(len(grid_matrix[i])):
                ...
                self.create_object((i * self.TILE_SIZE[0], j * self.TILE_SIZE[1]),
                                   self.objects.get(self.grid_matrix[i][j]), self.TILE_SIZE)
    ...
    def spawn_hazards(self):
        ...
        while counter != self.MINES_NUM:
            ...
            if (mine_pos not in self.mines.keys()) and (mine_pos != [0, 0]) and (
        ...
        while counter != self.GRANATS_NUM:
            ...
            granat_pos = (l, m)
            if (granat_pos not in self.mines.keys()) and (granat_pos not in self.granats.keys()) and (
                    granat_pos != [0, 0]) and (
                    self.grid_matrix[l // 60][m // 60] == 0 or self.grid_matrix[l // 60][m // 60] == 1):
    ...
    def create_object(self, position, object_name, object_size):
        self.window.blit(resize_image(f"./sprites/{object_name}", f"./temporaryFiles/{object_name}", object_size),
                         position)
